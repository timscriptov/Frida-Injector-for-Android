...
public class TouchNavigationMethod extends GestureDetector.SimpleOnGestureListener {
    ...
    public TouchNavigationMethod(@NotNull FreeScrollingTextField textField) {
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX,
        ...
        if (isCaretTouched) {
            ...
            //mTextField.smoothScrollBy((int)distanceX, (int)distanceY);
...
//    }
    ...
    protected void scrollView(float distanceX, float distanceY) {
        ...
        int maxWidth = Math.max(mTextField.getMaxScrollX(),
                mTextField.getScrollX());
        if (newX > maxWidth) {
            newX = maxWidth;
        } else if (newX < 0) {
            newX = 0;
        ...
        int maxHeight = Math.max(mTextField.getMaxScrollY(),
                mTextField.getScrollY());
        if (newY > maxHeight) {
            newY = maxHeight;
        } else if (newY < 0) {
            newY = 0;
    ...
    private boolean onTouchZoom(@NotNull MotionEvent e) {
    ...
    final protected int getPointerId(@NotNull MotionEvent e) {
